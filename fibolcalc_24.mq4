   ...
   if(Ask > LastClose) 
     {
      ...
      //tp1 = Ask +30*Point;
       ...
       //Added to correct re-entry after first take-profit.
       double diff1 = tp1 - fb;
       double diff2 = tp1 - Ask;
       //----
       if(ObjectFind("fb") != 0)
         {
       ...
       if((ra1 > 2) && (ra2 > 2) && (ra3 > 2)&& (Ask >= fb)){
        double fprice = Ask;
        tp1 = Ask + 30*Point+ spread;
        tp1 = NormalizeDouble(tp1,Digits);
        fe = OrderOpenPrice()- 20*Point+ spread;
           ...
           " Risk/Reward Ratio : ", ra3 ," OK Trade ");
             ...
             res = OrderSend(Symbol(),OP_BUY,Lots,Ask,3,0,tp1,"My order",MAGICMA,0,Blue);
              opbuy = true;
     ...
     // tp1 = Ask-30*Point;
      ...
      /* if(ObjectFind("fs") != 0)
       ...
       if((ra1 > 2) && (ra2 > 2) && (ra3 > 2) && (Bid <= fs)){
        double sPrice = Bid;
       tp1 = Bid-30*Point + spread;
       NormalizeDouble(tp1,Digits);
       fe = OrderOpenPrice()+ 25 *Point + spread;
           Comment("Owner : ", AccountName(),"Account number : ", AccountNumber(),
           "\n\nPrevDayHi ",PrevDayHi,"\nPrevDayLo ", PrevDayLo,"\nTrend was Down ",
           "\nSELL @ ",fs ,"\nStopLoss ",fe,"\nTakeProit 1 ",tp1 ,
           " Risk/Reward Ratio : ", ra1 ," OK Trade ","\nTakeProit 2 ",tp2 ,
           " Risk/Reward Ratio : ", ra2 ," OK Trade ","\nTakeProit 3 ",tp3,
           " Risk/Reward Ratio : ", ra3 ," OK Trade ");
          res=OrderSend(Symbol(),OP_SELL,Lots,Bid,3,0,tp1,"My order",MAGICMA,0,Yellow);
          opsell = true;
           }
       else
           Comment("Owner : ", AccountName(),"Account number : ", AccountNumber(),
