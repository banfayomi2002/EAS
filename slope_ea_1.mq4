...
//---- Calculate Pivots
 ...
 takeprofit1 = Ask +  takeprofit*Point+ spread;
 takeprofit2 = Bid -  ((takeprofit*Point)+ spread);
 //////////////////////////////////////////////////////////////
 ...
 //       Print("Bar date/time: ", TimeToStr(Time[x]), " close: ", Close[x], " vect[", x, "] = ", vect[x], " 2*WMA(p/2) = ", 2*WMA(x, period/2), " WMA(p) = ",  WMA(x, period)); 
   ...
   for(x = e-period; x >= 0; x--)
    ...
    { Uptrend[x] = ExtMapBuffer[x]; 
      ...
      res = OrderSend(Symbol(),OP_BUY,Lots,Ask,3,0,takeprofit1,"My order",MAGICMA,0,Blue);
      res +=1;
    }
    else              
    if (trend[x]<0)
    { 
      ...
      if (trend[x+1]>0) Dntrend[x+1]=ExtMapBuffer[x+1];
         ...
         res=OrderSend(Symbol(),OP_SELL,Lots,Bid,3,0,takeprofit2,"My order",MAGICMA,0,Red);
         res +=1;
    }       
         ...
         if(trend[x]<0)
           ...
           if(!OrderClose(OrderTicket(),OrderLots(),Bid,3,White))
               Print("OrderClose error ",GetLastError());
                 OrderSend(Symbol(),OP_SELL,Lots,Bid,3,0,takeprofit2,"My order",MAGICMA,0,Red);
           }
         break;
        }
      if(OrderType()==OP_SELL)
        {
         ...
         if(trend[x]>0)
           ...
           if(!OrderClose(OrderTicket(),OrderLots(),Ask,3,White))
              Print("OrderClose error ",GetLastError());
             OrderSend(Symbol(),OP_BUY,Lots,Ask,3,0,takeprofit1,"My order",MAGICMA,0,Blue);
           }
         break;
        }
     }
